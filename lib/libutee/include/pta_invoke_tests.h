...
/*
 * Optee send a request to a virtual copro and copro replies , parameter b
 * provides the number of sent request
 */
#define PTA_MBOX_TEST_SEND_RECEIVE     0
/*
 * Optee driver reply to a response from copro  , the copro sends a request
 * at boot . when pta call is received, the tests checks that one request is
 * received , and it replies to the request and then copro sends another
 * request ... , b provide the number of response sent to the copro
 */
...
/* Test invalid send command , b provides the invalid send parameter test */
